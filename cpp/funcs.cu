...
__device__ Real volumetetr(Vec3 p1, Vec3 p2, Vec3 p3, Vec3 p4)
{
    ...
    Real vol;
    vol = fabs(dot(p4-p1, cross(p2-p1, p3-p1))) / 6.0f;
    return vol;
}
...
{
    Vec3 result;
    result.x = sqrt(1.0/h.x);
    result.y = sqrt(1.0/h.y);
    result.z = sqrt(1.0/h.z);
...
{
    Vec3 result;
    result.x = sqrt(1.0/(h.x*h.x*h.x));
    result.y = sqrt(1.0/(h.y*h.y*h.y));
    result.z = sqrt(1.0/(h.z*h.z*h.z));
