...
__constant__ Real legendre[LEGENDRE_COLS][LEGENDRE_COLS];
__constant__ Real nwElem[nwElem_shape0][4];
__constant__ int combinations[combinations_shape0][3];


void elems_set_constant_mem(std::vector<Real> legendreCoefs, std::vector<Real> nw_elem, std::vector<int> basisCombinations)
{
    cudaMemcpyToSymbol(legendre, legendreCoefs.data(), legendreCoefs.size() * sizeof(decltype(legendreCoefs)::value_type));
    cudaMemcpyToSymbol(nwElem, nw_elem.data(), nw_elem.size() * sizeof(decltype(nw_elem)::value_type));
    cudaMemcpyToSymbol(combinations, basisCombinations.data(), basisCombinations.size() * sizeof(decltype(basisCombinations)::value_type));
}

extern "C" __global__ void assembleElems(int NT,
                                         int Nbasis,
                                         int Ngauss,
                                         Real nodes[][3],
                                         Real minBoundBox[][6],
                                         int tetrahedrons[][4],
                                         int *tetrahedrons2elem,
                                         int *NbasisCummulative,
                                         Real *Aval,
                                         int *Aindices,
                                         int *Aindptr)
{
    ...
    for (i = 0; i < Nbasis; i++) {
        for (j = 0; j < Nbasis; j++) {
            ...
            if (pos == -1) {
                printf("Element integrals. Negative position.\n");
                break;
            }

            atomicAdd(&Aval[pos], sumA);
        }
    }
}
